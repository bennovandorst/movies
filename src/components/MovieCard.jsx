...
const MovieCard = ({ movie }) => {
    ...
    useEffect(() => {
        ...
        setIsInWatchlist(watchlist.some(item => item.id === movie.id && item.type === "movie"));
    }, [movie.id]);

    const toggleWatchlist = (e) => {
        e.preventDefault();
        const watchlist = JSON.parse(localStorage.getItem("watchlist") || "[]");
        const isInList = watchlist.some(item => item.id === movie.id && item.type === "movie");

        if (isInList) {
            const updatedWatchlist = watchlist.filter(item => !(item.id === movie.id && item.type === "movie"));
            localStorage.setItem("watchlist", JSON.stringify(updatedWatchlist));
        } else {
            const updatedWatchlist = [...watchlist, { id: movie.id, type: "movie", addedAt: new Date().toISOString() }];
            localStorage.setItem("watchlist", JSON.stringify(updatedWatchlist));
        }
